...
int main(int argc, char **argv){
	...
	//double delta=deltah(gitter, 4,0,laenge, j); //Ausprobieren von flipspin
	//printf("dH=%f\n", delta);
...
//	printf("%d %d %d %d %d %d\n", -1%4, -2%4, -3%4, -4%4, 4%4, 5%4); //Ausprobieren modulo
//	printf("%d %d %d %d %d %d\n", mod(-1,4), mod(-2,4), mod(-3,4), mod(-4,4), mod(4,4), mod(5,4)); //Ausprobieren modulo
...
//gibt Ausgaben wie diese: -> break vergessen
 //~ - o- o+ - o+ - o
 //~ - o- o+ - o+ - o
 //~ - o- oo- o
 //~ + - o- o+ - o- o
...
//~ void ausgabepm(int *gitter, int laenge){
	//~ //gibt ein laenge *laenge quadratisches Gitter auf die Standardkonsole aus
...
//~ double hamiltonianohnemod(int *gitter, int laenge, double j){
	//~ //berechnet den hamiltonian eines laenge*laenge quadratischen Gitters eines Ising Modells mit periodischen Randbedingungen
	//~ double H=0;
	//~ for (int d1=0; d1<laenge; d1+=1){//geht in erster dimension durch (Zeile
		//~ for (int d2=0; d2<laenge; d2+=1){//geht in zweiter dimension durch (alle Spalten einer Zeile)
